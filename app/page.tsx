...
const randomQuotes = [
  "Hey there! 👋",
  "Move ur mouse around and I'll follow you",
  "I'm just a simple animated face",
  "Press H to see some magic",
  "Of course I blink, my eyes are gonna get dry",
  "This was made with Next.js and Tailwind CSS",
];
...
function AnimatedEyes() {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event: MouseEvent) => {
      ...
      animationFrame = requestAnimationFrame(() => {
        const x =
          (event.clientX * 100) / window.innerWidth - 50 + CENTER_OFFSET;
        const y =
  ...
  return (
    <div className="pointer-events-none fixed inset-0 flex items-center justify-center">
      {/* GITHUB ICON */}
      <a
        href="https://github.com/Awhalen1999/animated-eyes"
        target="_blank"
        rel="noopener noreferrer"
        className="pointer-events-auto fixed top-4 right-4 z-50 text-zinc-400 transition-colors hover:text-zinc-200"
      >
        <svg
          className="h-6 w-6"
          fill="currentColor"
      ...
      <div
        className={`relative transition-all duration-300 ${
          isJumping ? "-translate-y-8 transform" : ""
        } ${
          isTilted
            ? tiltDirection === "left"
              ? "-rotate-12"
      ...
      >
        ...
        <div className="flex gap-20">
          {/* LEFT EYE */}
          <div
            className={`relative w-60 overflow-hidden rounded-full bg-white transition-all duration-300 ease-out ${
      ...
      <div className="pointer-events-none fixed right-6 bottom-8">
        <div className="flex flex-col items-end gap-2">
          {allQuotes.map((quote, index) => (
            <div
              key={quote.id}
              className="relative w-fit max-w-xs rounded-lg border border-zinc-700 bg-zinc-800 px-4 py-3 shadow-lg transition-all duration-200 ease-out"
              style={{
            ...
            >
              <p className="text-sm font-medium text-zinc-300">{quote.text}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <Popover open={showHelpMenu} onOpenChange={setShowHelpMenu}>
        <PopoverTrigger asChild>
          <div className="pointer-events-none fixed bottom-4 left-4">
            <p className="text-sm text-zinc-300 opacity-60">
        ...
        <PopoverContent className="w-80" side="top" align="start">
          <div className="space-y-2">
            <h4 className="text-lg font-bold mb-4 leading-none text-zinc-100">
